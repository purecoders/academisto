        ...
        <div class="col-md-6 mt-3">
            <form action="{{route('user-finance-update')}}" method="post">
                <div class="form-group">
                    ...
                    <input type="number" id="card-no" value="@if($userFullInfo){{$userFullInfo->bank_card_id}}@endif"
                           name="bank_card_id" class="form-control" placeholder="شماره کارت خود را بنویسید">
                </div>
                <div class="form-group">
                    <lable for="account-no"> شماره حساب</lable>
                    <input type="number" id="account-no"
                           value="@if($userFullInfo){{$userFullInfo->bank_account_id}}@endif" name="bank_account_id"
                           class="form-control" placeholder=" شماره حساب خود را بنویسید">
                </div>
                <div class="form-group">
                    <lable for="account-name">به نام</lable>
                    <input id="account-name" value="@if($userFullInfo){{$userFullInfo->bank_account_owner_name}}@endif"
                           name="bank_account_owner_name" class="form-control" placeholder="نام دارنده حساب را بنویسید">
                </div>

                {{csrf_field()}}
                @if($userFullInfo!==null)
                    <input type="submit" class="form-control btn btn-secondary" value="ویرایش ">
                @else
