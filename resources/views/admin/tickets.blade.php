...
@section('content')
    <div class="container rtl">
        <div class="row">
            <div class="col-lg-4 p-0  bg-dark">
                               ...
                               style="text-decoration: none">
                                <div class="admin-chat-view-user p-1 text-center bg-dark border-bottom border-info">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <h5 class="admin-chat-view-user-name mr-auto ">{{$user_ticket['user']['name']}}</h5>
                                        ...
                                        <div class="d-flex flex-column align-items-end justify-content-around">
                                            ...
                                            <form action="{{route('admin-user-profile')}}" method="post">
                                                ...
                                                <input type="hidden" name="user_id"
                                                       value="{{$user_ticket['user']['id']}}">
                                                @if($user_ticket['new_tickets_count'] > 0)
                                                    <span class="dot p-2 bg-danger">{{$user_ticket['new_tickets_count']}}</span>
                                                @endif
            ...
            <div class="col-lg-8 p-0 mb-0">
                <div class="admin-chat-view-user-header p-2 ml-1">
                    <h4>تیکت ها</h4>
                </div>
                <div class="card-body bg-dark m-0 p-4 ml-1 ">
                    <div id="chat">
                ...
                <div class="card-footer d-flex ml-1 bg-dark">
                    <form id="admin-send-ticket-form" class="d-flex flex-grow-1" action="{{route('admin-send-ticket')}}"
                          method="post">
                        {{csrf_field()}}
                        ...
                        @if($special_user_tickets !== null && $special_user_tickets!= [])
                            <input id="send-ticket-user-id" type="hidden" name="user_id" value="{{$ticket->user->id}}">
                        @else
                            <input id="send-ticket-user-id" type="hidden" name="user_id" value="null">
                        @endif
                        <button class="btn btn-primary align-self-center" type="submit" onclick="sendFromAdmin(this)">
                            ارسال
                        </button>
